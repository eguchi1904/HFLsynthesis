...
let counter = ref 0
let gen_string (s:Id.t) =
  incr counter;
  Printf.sprintf "%s.%d" (Id.to_string s) !counter
...
let rec formula2z3 (ctx:context) (smap:sort_map) (emap:id_expr_map) (fmap:id_fun_map) (e:Formula.t) 
  ...
  |Formula.Var (sort, i) ->
    ...
    (try
     ...
     with
      Not_found ->
      let new_var = (Expr.mk_const_s ctx (Id.to_string i) z_sort) in
  ...
  |Formula.Cons (sort, i , args) ->
    ...
    (try 
     ...
     with
       Not_found ->
       let new_fun = (FuncDecl.mk_func_decl_s ctx (Id.to_string i) z_args_s z_sort) in
