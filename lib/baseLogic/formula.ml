...
let rec sort2string = function
  ...
  |IntS -> "Int"
  |DataS (i,sorts) ->
    Printf.sprintf "%s %s" (Id.to_string i) (String.concat " " (List.map sort2string sorts))
  |SetS s -> Printf.sprintf "Set %s" (sort2string s)
  |AnyS i -> (Id.to_string i)
  |UnknownS i -> Printf.sprintf "unknown_%s" (Id.to_string i)
...
let of_string senv = 
  let senv_str =
    String.concat
      ...
      (List.map
         (fun (x, sort) ->
           Printf.sprintf "%s: %s" (Id.to_string x) (sort2string sort))
          senv)
    in
    Printf.sprintf "\n--------------------------------------------------\n%s\n--------------------------------------------------\n"
    senv_str
...
let rec p2string e =
 ...
 |Unknown (senv, _, sita, id)->
    ...
    let sita_str_list = List.map
                          (fun (s, p) -> Printf.sprintf "%s->%s" (Id.to_string s) (p2string p))
                          sita_list
    in
    let senv_str = List.map fst (Senv.reveal senv)
                   |> List.map (Id.to_string_readable)
                   |> String.concat "," in
    if sita_list = [] then
      Printf.sprintf "senv:[%s].P[%s]" senv_str (Id.to_string_readable id)
      (* Printf.sprintf "P[%s]" id *)
    else
      (* Printf.sprintf "sita:[%s].P[%s]" (String.concat ";" sita_str_list) id *)
      Printf.sprintf "senv:[%s].sita:[%s].P[%s]" senv_str (String.concat ";" sita_str_list) (Id.to_string_readable id)
  |Cons (_,id,ts)|UF (_,id,ts) ->
    let ts_string = String.concat " " (List.map p2string ts) in
    Printf.sprintf "(%s %s)" (Id.to_string_readable id) ts_string
  |All (args,t) ->
...
let rec p2string_with_sort = function
  ...
  |Var (s,id) ->Printf.sprintf "(%s:%s) " (Id.to_string_readable id) (sort2string s)
  |Unknown (_, sort_sita, sita, id)->
    let sita_list = M.bindings sita
                  |> List.map (fun (a,b) -> (Id.to_string_readable a, b))
    in
    let sita_str_list = List.map
    ...
    let sort_sita_str =
      ...
      |> List.map
           (fun (s, p) -> Printf.sprintf "%s -> %s" (Id.to_string_readable s) (sort2string p))
      |> String.concat ";" 
    in
    if sita_list = [] && sort_sita = M.empty then
      Printf.sprintf "P[%s]" (Id.to_string_readable id)
    else
      Printf.sprintf "[%s][%s].P[%s]" sort_sita_str (String.concat ";" sita_str_list) (Id.to_string_readable id)
  |Cons (s,id,ts)|UF (s,id,ts) ->
    let ts_string = String.concat " " (List.map p2string_with_sort ts) in
    Printf.sprintf "((%s %s):%s)" (Id.to_string_readable id) ts_string (sort2string s)
  |All (args,t) ->
...
let rec any2unknownsort s =
  ...
  let sita = S.fold
               (fun any_id sita -> M.add any_id (UnknownS (Id.genid (Id.to_string_readable any_id))) sita)
               any_ids
               M.empty
  in
  sort_subst sita s
